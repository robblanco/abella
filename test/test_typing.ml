    ...
    [
        ...
        (fun () ->
           ...
           in
             assert_raises
               ...
               (fun () -> type_uclause ~sr:(default_sr ()) ~sign:(default_sign ()) uclause) (*NOTE OK? *)
        );
        ...
        (fun () ->
           ...
           in
             assert_raises
               ...
               (fun () -> type_udef ~sr:(default_sr ()) ~sign:(default_sign ()) udef) (*NOTE OK? *)
        );
      ...
      "Should not allow quantification over prop in metaterm" >::
        (fun () ->
           ...
           in
             assert_raises
               (Failure "Cannot quantify over type prop")
               (fun () -> type_umetaterm ~sr:(default_sr ()) ~sign:(default_sign ()) umetaterm) (*NOTE OK? *)
        );
      ...
      "Should replace underscores in clauses with fresh names" >::
        (fun () ->
           ...
           in
             let clause = type_uclause ~sr:(default_sr ()) ~sign:(default_sign ()) uclause in (*NOTE OK? *)
